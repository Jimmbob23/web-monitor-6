...
app = FastAPI(title="Web Monitor Enterprise 6", version="6.0.1", lifespan=lifespan)
...
def api_stats(db: Session = Depends(get_db)):
    return {"sites": db.query(Site).count(), "enabled": db.query(Site).filter(Site.enabled == True).count(), "changed": db.query(Change).filter(Change.status == "changed").count(), "errors": db.query(Change).filter(Change.status == "error").count()}
...
def dashboard(request: Request, q: str = "", db: Session = Depends(get_db)):
    ...
    stats = {"sites": db.query(Site).count(), "enabled": db.query(Site).filter(Site.enabled == True).count(), "changed": db.query(Change).filter(Change.status == "changed").count(), "errors": db.query(Change).filter(Change.status == "error").count()}
...
@app.post("/sites")
def create_site(request: Request, name: str = Form(""), url: str = Form(...), folder_name: str = Form(""), tags_csv: str = Form(""), schedule_type: str = Form("interval"), interval_seconds: int = Form(300), cron_expression: str = Form(""), threshold_percent: float = Form(0.5), wait_seconds: int = Form(2), viewport_width: int = Form(1440), viewport_height: int = Form(1200), ignore_selectors: str = Form(""), cookie_mode: str = Form("auto"), db: Session = Depends(get_db)):
    redirect = require_login(request, db)
    if redirect:
        return redirect
    folder = None
    if folder_name.strip():
        ...
        if not folder:
            ...
            db.add(folder)
            db.flush()
    site = Site(name=name.strip() or normalize_url(url), url=normalize_url(url), folder_id=folder.id if folder else None, tags_csv=tags_csv.strip(), schedule_type=schedule_type, interval_seconds=max(interval_seconds, 60), cron_expression=cron_expression.strip(), threshold_percent=max(threshold_percent, 0), wait_seconds=max(wait_seconds, 0), viewport_width=max(viewport_width, 320), viewport_height=max(viewport_height, 320), ignore_selectors=ignore_selectors.strip(), cookie_mode=cookie_mode, enabled=True)
    db.add(site)
    db.commit()
    sync_jobs()
...
def edit_site(site_id: int, request: Request, name: str = Form(""), url: str = Form(...), tags_csv: str = Form(""), schedule_type: str = Form("interval"), interval_seconds: int = Form(300), cron_expression: str = Form(""), threshold_percent: float = Form(0.5), wait_seconds: int = Form(2), viewport_width: int = Form(1440), viewport_height: int = Form(1200), ignore_selectors: str = Form(""), cookie_mode: str = Form("auto"), enabled: str | None = Form(None), db: Session = Depends(get_db)):
    redirect = require_login(request, db)
    if redirect:
        return redirect
    site = db.get(Site, site_id)
